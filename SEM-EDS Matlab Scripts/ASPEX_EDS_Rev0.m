% Aspex Explorer SEM-EDS TIFF image data extractor
%
% Searches the current directory for all .TIFF files and plots SEM images with scale bars.
% If EDS data is present in the TIFF file it is graphed and the sampling area is marked
% (only works for area scans, not point scans).
%
% Written by Kristofer Weisshaupt (kris.weisshaupt@maplaboratory.net)
%
% Tested in Matlab 2016, may require some modification for later versions.
% Setting the color of the HArea plot is a known issue in later Matlab releases.
...
% Enter the atomic symbols for any peaks you want labelled in the EDS
% spectra:
elements={ 'C', 'K', 'Na', 'Ca', 'Mg' 'Ti', 'Fe', 'Al', 'Si', 'P', 'S', 'O'};

% Set x-axis bounds for EDS spectra plots:
xMin= 0;
xMax=10;

% Select all .TIF files in the current directory.
files = dir('*.TIF');


%results = {'Sample','O','Al','Si', 'Ca', 'Cr', 'Fe', 'Ni'};

% Reference list of element emmision lines:
lines= {
...
for file = files'
    ...
    clearvars -except files file results lines xMin xMax elements
    ...
    %set(figure(1), 'Position', [0 0 width*2 height*2]);
    % set(figure(1), 'Position', [0 0 512*1.8 512*2]);
    ...
    if ~all(pixUnits  < 128)
        pixUnits = [char(181) 'm'];%'\mum';
    end
    
    close all;
    
    
    location=strfind(desc,'Accelerating_Voltage=');
    voltage=desc(location(1,1)+21:location(1,1)+27);
    
    location=strfind(desc,'Working_Distance=');
    workingDistance=desc(location(1,1)+17:location(1,1)+23);
    ...
    if isempty(location)
        ...
        image = [image; zeros(round(height/12),width)];
        
        newHeight=height+round(height/12);
        imshow(image, 'XData', [0 width*pixelSize], 'YData', [0 newHeight*pixelSize])
        ...
        set(gcf, 'PaperPosition', [0 0 width/300 newHeight/300]);    % can be bigger than screen
        
        scaleHeight = height*pixelSize*1/100;
        scaleWidth = round(width*pixelSize*1/4,1,'significant');
        scaleLabel=sprintf('%d %s', scaleWidth, pixUnits);
        if(scaleWidth<1)
            scaleLabel=sprintf('%d %s', scaleWidth*1000, 'nm');
        end
        
        
        scaleX= width*pixelSize-scaleWidth-width*pixelSize/80;
        scaleY= (height+round(height/80))*pixelSize;
        rectangle('Position',[scaleX,scaleY,scaleWidth,scaleHeight],'FaceColor','w','EdgeColor','w')
        rectangle('Position',[scaleX,scaleY,scaleHeight/1.2,scaleHeight*3],'FaceColor','w','EdgeColor','w')
        rectangle('Position',[scaleX+scaleWidth*1/5-scaleHeight/4,scaleY,scaleHeight/2,scaleHeight*1.75],'FaceColor','w','EdgeColor','w')
        rectangle('Position',[scaleX+scaleWidth*2/5-scaleHeight/4,scaleY,scaleHeight/2,scaleHeight*1.75],'FaceColor','w','EdgeColor','w')
        rectangle('Position',[scaleX+scaleWidth*3/5-scaleHeight/4,scaleY,scaleHeight/2,scaleHeight*1.75],'FaceColor','w','EdgeColor','w')
        rectangle('Position',[scaleX+scaleWidth*4/5-scaleHeight/4,scaleY,scaleHeight/2,scaleHeight*1.75],'FaceColor','w','EdgeColor','w')
        rectangle('Position',[scaleX+scaleWidth*5/5-scaleHeight/1.2,scaleY,scaleHeight/1.2,scaleHeight*3],'FaceColor','w','EdgeColor','w')
        % ylabel(['Distance (' pixUnits ')']);
        
        text(scaleX+scaleWidth/2, scaleY+scaleHeight*2.5, scaleLabel, ...
        ...
        text(width*0.01*pixelSize, height*1.005*pixelSize, sprintf('WD: %s',workingDistance), ...
        ...
        text(width*0.01*pixelSize, height*1.04*pixelSize, voltage, ...
            'HorizontalAlignment', 'left', ...
            'VerticalAlignment', 'top', ...
            'FontSize', height/150, ...
            'Color', 'w', ...
        ...
        text((width/2-width/20)*pixelSize, (height+round(height/24))*pixelSize, file.name(1:length(file.name)-4), ...
            'HorizontalAlignment', 'center', ...
        ...
        print(gcf, strcat(file.name(1:length(file.name)-4),'.png'), '-dpng', '-r300' );   %save file as PNG w/ 300dpi
        set(gcf, 'PaperSize', [width/300 newHeight/300])    % Same, but for PDF output
        print(gcf, strcat(file.name(1:length(file.name)-4),'.pdf'), '-dpdf', '-r300' );   %save file as PDF w/ 300dpi
...
set(hFigure, 'ToolBar', 'none');
        ...
        box(3)=box(3)-box(1);
        box(4)=box(4)-box(2);
        rectangle('Position',box,'FaceColor',[1,1,1,0],'LineWidth',1,'EdgeColor',[0,0,1,0.5]);
        ...
        set(gca,'units','pixels'); % set the axes units to pixels
        x1 = get(gca,'position'); % get the position of the axes
        set(gcf,'units','pixels'); % set the figure units to pixels
        y1 = get(gcf,'position'); % get the figure position
        set(gcf,'position',[y1(1) y1(2) x1(3) x1(4)]);% set the position of the figure to the length and width of the axes
        
        %set(gca,'units','normalized','position',[0 0 1 1]);
        %set(gcf, 'PaperPosition', [0 0 width/300 newHeight/300]);
        
        
        %set(gca,'units','normalized','position',[0 0.3 1 0.75]);
        %set(gcf, 'PaperPosition', [0 0 width/300 newHeight/300]);    % can be bigger than screen
        ...
        text((width/2-width/20)*pixelSize, (height+round(height/24))*pixelSize, file.name(1:length(file.name)-4), ...
            ...
            'FontWeight', 'bold');
             ...
             set(gca,'units','normalized','position',[0 0.3 1 0.70]);
        set(gcf, 'PaperPosition', [0 0 width/300 newHeight/300]);    % can be bigger than screen
        
        
        
        subplot('Position',[0.25, 0, 0.75, 0.05])
        ...
        for i=1:length(lines)
            ...
            if(~isempty(elements(strcmp(elements, cell2mat(lines(i,1))))))
                %if(cell2mat(lines(i,4))>15 && isempty(strfind(cell2mat(lines(i,3)),'Ll')) && isempty(strfind(cell2mat(lines(i,3)),'2')) && isempty(strfind(cell2mat(lines(i,3)),'L\beta')))
                if(cell2mat(lines(i,4))>7 && isempty(strfind(cell2mat(lines(i,3)),'K\alpha2')))      
                    h2=text(cell2mat(lines(i,2)),1.05*max(eds(find(energy>=cell2mat(lines(i,2)),1)-1:find(energy>=cell2mat(lines(i,2)),1)-1)),strcat(cell2mat(lines(i,1)),'_{',cell2mat(lines(i,3)),'}'),'Color','black','fontweight','normal','FontSize',height/200,'HorizontalAlignment','Center','VerticalAlignment','bottom');
                    h1E=h1.Extent;
                    h2E=h2.Extent;
                    h2P=h2.Position;
                    %if(h1E(1)+h1E(3)>h2E(1) && h1E(2)<h2E(4) && h1E(2)+h1E(4)<h2E(2)+h2E(4))
        ...
        xlabel('Energy (keV)','FontSize',height/180);
        
        ylabel('Count','FontSize',height/180);
        
        
        ax = gca; % axes handle
        ax.YAxis.Exponent = floor(log10(1.2*max(eds)));
        ytickformat('%.1f')
        ax.XAxis.FontSize = height/180;
        ax.YAxis.FontSize = height/180;
        ...
        desc=info(2).ImageDescription;
        
        comment=strfind(desc,'Comment=');
        comp='';
        if(~isempty(comment))
            zoom=max(comment)+strfind(desc(max(comment):length(desc)),'RasterBox');
            comment=desc(max(comment)+8:min(length(desc),max(comment)+100));
            comment=strrep(comment, ' ', '');
            %comment='98.2Al5Mg1Ni';
            baz = @(s,b)regexprep(s,'(\d+\.?\d*)','${num2str(str2double($1)*str2double(b))}'); %%%
            tmp = regexprep(comment,'\((([A-Z][a-z]*\d+\.?\d*)+)\)(\d+\.?\d*)','${baz($1,$2)}');   %%%
            comp = regexp(tmp,'(\d+\.?\d*)([A-Z][a-z]*)','tokens');
            comp = vertcat(comp{:})
            if(~isempty(comp))
                for n = 1:numel(comp(:,1))
                    text(0.875*xMax, 1.2*max(eds)-0.09*n*max(eds), sprintf('%s',cell2mat(comp(n+numel(comp(:,1))))),'Color','black','fontweight','bold','FontSize',height/200,'HorizontalAlignment','Left','VerticalAlignment','top');
                       text(0.925*xMax, 1.2*max(eds)-0.09*n*max(eds), sprintf('%0.1f',str2num(cell2mat(comp(n)))),'Color','black','fontweight','normal','FontSize',height/200,'HorizontalAlignment','Left','VerticalAlignment','top');
        ...
        set(gcf,'units','pixels'); % set the figure units to pixels
        y = get(gcf,'position'); % get the figure position
        set(gcf,'position',[y(1) y(2) x(3) x(4)+x1(4)]);% set the position of the figure to the length and width of the axes
        set(gca,'units','normalized','position',[0.08 0.05 0.90 0.22]);
        set(gcf, 'PaperPosition', [0 0 width/300 newHeight/0.7/300]);    % can be bigger than screen
        ...
        set(gcf, 'InvertHardCopy', 'off');
        print(gcf, strcat(file.name(1:length(file.name)-4),'_Tight.png'), '-dpng', '-r300' );   %save file as PNG w/ 300dpi
        set(gcf, 'PaperSize', [width/300 newHeight/0.7/300])    % Same, but for PDF output
        print(gcf, strcat(file.name(1:length(file.name)-4),'.pdf'), '-dpdf', '-r300' );   %save file as PDF w/ 300dpi
